#!/bin/bash
# Usage: build-test.sh [ubuntu-22.04|ubuntu-20.04|ubuntu-18.04|alpine-edge|fedora-36]...
...
ubuntu-22.04() {
    STIME=$(date +%s)
    echo "$(date) Build redbpf with kernel headers on Ubuntu 22.04"
    docker run --privileged -v $WORKDIR:/build -w /build $IMAGE_NAME:latest-ubuntu-22.04 /bin/bash -c 'cargo clean && export KERNEL_VERSION=$(ls --indicator-style=none /lib/modules/) && echo KERNEL_VERSION=$KERNEL_VERSION && cargo build && cargo build --examples --features=kernel5_8'
    ERR=$?
    ETIME=$(date +%s)
    echo "took $(((ETIME - STIME) / 60))m$(((ETIME - STIME) % 60))s"
    if [[ $ERR -ne 0 ]]; then
        echo "Failed to build redbpf with kernel headers on Ubuntu 22.04" >&2
        exit 1
    fi

    STIME=$(date +%s)
    echo "$(date) Build redbpf wih vmlinux on Ubuntu 22.04"
    docker run --privileged -v $WORKDIR:/build -w /build $IMAGE_NAME:latest-ubuntu-22.04 /bin/bash -c 'cargo clean && /lib/modules/*/build/scripts/extract-vmlinux /boot/vmlinuz > /boot/vmlinux && export REDBPF_VMLINUX=/boot/vmlinux && cargo build && cargo build --examples --features=kernel5_8'
    ERR=$?
    ETIME=$(date +%s)
    echo "took $(((ETIME - STIME) / 60))m$(((ETIME - STIME) % 60))s"
    if [[ $ERR -ne 0 ]]; then
        echo "Failed to build redbpf with vmlinux on Ubuntu 22.04" >&2
        exit 1
    fi
}
...
alpine-edge() {
    STIME=$(date +%s)
    echo "$(date) Build redbpf wih kernel headers on Alpine Edge"
    ...
    echo "took $(((ETIME - STIME) / 60))m$(((ETIME - STIME) % 60))s"
    if [[ $ERR -ne 0 ]]; then
        echo "Failed to build redbpf with kernel headers on Alpine Edge" >&2
        exit 1
    fi
